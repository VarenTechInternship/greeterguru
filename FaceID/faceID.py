    ...
    while True:
        ...
        for(x,y,w,h) in faces:
            ...
            if ((confidence < 100) and (100-confidence)>25):
                ...
                cv2.putText(img, "conf: "+str(confidence), (x+w+25,int(y+(h/2))+30), font, .75, (255,255,0), 2)  # prints confidence level
            else:
                msg = "???"
                confidence = "{0}%".format(round(100 - confidence))
                #cv2.rectangle(img, (x,y), (x+w,y+h), (0,255,0), 6) # outlines face with rectangle
                ...
                cv2.putText(img, "conf: "+str(confidence), (x+w+25,int(y+(h/2))+30), font, .75, (255,255,255), 2)  # prints confidence level        
